...
const Cart = () => {
    const { products, currency, cartItems, updateQuantity, delivery_fee, clearCart, loading } = useContext(ShopContext);
    const [cartData, setCartData] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        if (loading) return; // Don't proceed if products are still loading
